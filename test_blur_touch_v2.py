...
def create_blur_touch_v2_video(input_video, output_path):
    """
    Version 2: More zoomed in (120%)
    - 640px blur sections
    - Middle video zoomed to 120% for tighter crop
    """
    ...
    print(f"Touch V2: 120% zoom, blur={blur_section_height}px")
    subprocess.run(cmd, check=True)

def test_blur_touch_v2():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            final_path = f"output/{safe_name}_TOUCH_V2_120zoom.mp4"
            
            print(f"Testing Touch V2 with {player_name}...")
            download_youtube_video(youtube_url, youtube_path, duration=10)
            create_blur_touch_v2_video(youtube_path, final_path)
            
            if os.path.exists(youtube_path):
                os.remove(youtube_path)
            
            print(f"✅ Touch V2: {final_path}")
            return final_path
