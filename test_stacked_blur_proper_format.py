...
def create_proper_stacked_blur_video(input_video, output_path):
    """
    Create stacked blur effect with proper Instagram format middle video
    - Top blur section (same footage blurred)
    - Middle section with properly formatted main video (using Instagram format logic)
    - Bottom blur section (same footage blurred)
    """
    ...
    cmd = [
        ...
        '-filter_complex', (
            ...
            # Create overlay video with proper Instagram format scaling (from main script)
            # Scale to fit height, then pad with black bars - this ensures proper aspect ratio
    ...
    print(f"Creating proper stacked blur effect with Instagram format...")
    print(f"Middle section dimensions: {width}x{main_section_height}")
    print(f"Input: {input_video}")
    print(f"Output: {output_path}")
    
    subprocess.run(cmd, check=True)
    print(f"✅ Proper stacked blur video created: {output_path}")
...
def concatenate_videos(intro_path, main_path, output_path):
    ...
    print("Concatenating intro with proper stacked blur video...")
    try:
        subprocess.run(cmd, check=True)
    finally:
...
def process_wr_with_proper_stacked_blur(player_name, team, fantasy_points, rank, youtube_url, output_dir="output"):
    """Complete workflow: intro + proper stacked blur highlights"""
    ...
    stacked_path = f"{output_dir}/{safe_name}_proper_stacked_blur.mp4"
    final_path = f"{output_dir}/{safe_name}_PROPER_STACKED_FINAL.mp4"
    
    print(f"\n🎬 Processing {player_name} with proper stacked blur effect...")
    ...
    # Step 3: Create proper stacked blur version
    create_proper_stacked_blur_video(youtube_path, stacked_path)
    
    # Step 4: Concatenate intro + proper stacked blur video
...
def test_with_csv_data():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            print(f"\n🏈 Testing proper stacked blur with: {player_name} (#{rank})")
            
            try:
                final_video = process_wr_with_proper_stacked_blur(
                    player_name, team, fantasy_points, rank, youtube_url
                )
                print(f"\n✅ Success! Proper stacked blur video: {final_video}")
                return final_video
...
if __name__ == "__main__":
    print("Testing proper stacked blur effect...")
    test_with_csv_data()