...
def create_blur_option4_video(input_video, output_path):
    """
    Option 4: Maximum blur, minimal middle
    - 800px blur sections (top/bottom)
    - 320px middle section
    """
    
    width, height = 1080, 1920
    
    blur_section_height = 800  # Top and bottom blur sections
    main_section_height = 320  # Center section
    ...
    print(f"Option 4: Blur={blur_section_height}px, Middle={main_section_height}px")
...
def test_option4():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            final_path = f"output/{safe_name}_OPTION4_800blur_320middle.mp4"
            
            print(f"Testing Option 4 with {player_name}...")
            download_youtube_video(youtube_url, youtube_path, duration=10)
            create_blur_option4_video(youtube_path, final_path)
            
            if os.path.exists(youtube_path):
                os.remove(youtube_path)
            
            print(f"✅ Option 4: {final_path}")
            return final_path
