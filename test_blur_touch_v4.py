...
def create_blur_touch_v4_video(input_video, output_path):
    """
    Version 4: Maximum zoom (140%)
    - 640px blur sections
    - Middle video zoomed to 140% for close-up action
    """
    ...
    print(f"Touch V4: 140% zoom, blur={blur_section_height}px")
    subprocess.run(cmd, check=True)

def test_blur_touch_v4():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            final_path = f"output/{safe_name}_TOUCH_V4_140zoom.mp4"
            
            print(f"Testing Touch V4 with {player_name}...")
            download_youtube_video(youtube_url, youtube_path, duration=10)
            create_blur_touch_v4_video(youtube_path, final_path)
            
            if os.path.exists(youtube_path):
                os.remove(youtube_path)
            
            print(f"✅ Touch V4: {final_path}")
            return final_path
