...
def create_blur_option1_video(input_video, output_path):
    """
    Option 1: Original good ratio
    - 640px blur sections (top/bottom)
    - 640px middle section
    """
    
    width, height = 1080, 1920
    
    blur_section_height = 640  # Top and bottom blur sections
    main_section_height = 640  # Center section
    ...
    print(f"Option 1: Blur={blur_section_height}px, Middle={main_section_height}px")
...
def test_option1():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            final_path = f"output/{safe_name}_OPTION1_640blur_640middle.mp4"
            
            print(f"Testing Option 1 with {player_name}...")
            download_youtube_video(youtube_url, youtube_path, duration=10)
            create_blur_option1_video(youtube_path, final_path)
            
            if os.path.exists(youtube_path):
                os.remove(youtube_path)
            
            print(f"✅ Option 1: {final_path}")
            return final_path
