...
def create_blur_touch_v1_video(input_video, output_path):
    """
    Version 1: Based on Option 1 but with blur touching middle video
    - 640px blur sections
    - Middle video zoomed to 110% to fill gaps
    """
    ...
    # Zoom the middle video by 110% to ensure it touches blur edges
    zoom_factor = 1.1
    ...
    print(f"Touch V1: 110% zoom, blur={blur_section_height}px")
    subprocess.run(cmd, check=True)

def test_blur_touch_v1():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            final_path = f"output/{safe_name}_TOUCH_V1_110zoom.mp4"
            
            print(f"Testing Touch V1 with {player_name}...")
            download_youtube_video(youtube_url, youtube_path, duration=10)
            create_blur_touch_v1_video(youtube_path, final_path)
            
            if os.path.exists(youtube_path):
                os.remove(youtube_path)
            
            print(f"✅ Touch V1: {final_path}")
            return final_path
