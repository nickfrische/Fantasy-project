...
def create_blur_option3_video(input_video, output_path):
    """
    Option 3: Less blur, bigger middle
    - 560px blur sections (top/bottom)
    - 800px middle section
    """
    
    width, height = 1080, 1920
    
    blur_section_height = 560  # Top and bottom blur sections
    main_section_height = 800  # Center section
    ...
    print(f"Option 3: Blur={blur_section_height}px, Middle={main_section_height}px")
...
def test_option3():
    ...
    for row in rows:
        if row['Posted'].lower() == 'false':
            ...
            final_path = f"output/{safe_name}_OPTION3_560blur_800middle.mp4"
            
            print(f"Testing Option 3 with {player_name}...")
            download_youtube_video(youtube_url, youtube_path, duration=10)
            create_blur_option3_video(youtube_path, final_path)
            
            if os.path.exists(youtube_path):
                os.remove(youtube_path)
            
            print(f"✅ Option 3: {final_path}")
            return final_path
